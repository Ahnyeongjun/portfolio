...
description: "Bloom, AO, ToneMapping 등 포스트 프로세싱 효과로 3D 렌더링 품질을 높이는 방법을 정리한다."
date: "2025-02-05"
category: "Three.js"
...
3D 렌더링 결과물을 후처리하여 시각적 품질을 높인다. 2D 이미지 보정과 유사하게, 렌더링된 프레임에 효과를 적용한다.
...
밝은 영역이 빛나는 것처럼 보이게 한다. 금속 하이라이트, 발광 재질에 효과적이다.
...
틈새와 모서리에 자연스러운 그림자를 추가한다. "접촉 그림자"라고도 한다.
...
N8AO는 SSAO의 개선 버전으로, 품질과 성능의 균형이 좋다.

---

## ToneMapping

HDR 렌더링 결과를 모니터가 표시할 수 있는 LDR 범위로 변환한다.
...
ACES Filmic은 하이라이트가 자연스럽게 롤오프되고 색감이 좋다.

---

## EffectComposer 구성

여러 효과를 조합할 때 `EffectComposer`를 사용한다.
...
포스트 프로세싱과 함께 조명 설정도 중요하다. 단일 조명으로는 입체감이 부족하다.
...
`shadow-mapSize`를 높이면 선명해지지만 VRAM을 더 사용한다. 2048은 품질과 성능의 균형점이다.

---

## 실시간 파라미터 조절

Zustand로 파라미터를 관리하고 UI로 조절한다.
