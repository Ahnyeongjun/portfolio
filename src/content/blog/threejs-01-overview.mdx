...
description: "웹 기반 3D 뷰어 시스템을 구현하며 사용한 기술 스택과 선택 이유를 정리한다."
date: "2025-02-05"
category: "Three.js"
...
웹 기반 3D 뷰어 시스템을 구현하며 사용한 기술들을 정리한다.
...
R3F는 Three.js 객체를 React 컴포넌트로 추상화한다. 메모리 관리, 리렌더링 최적화를 React에 위임할 수 있어 코드가 간결해진다.

**2. 상태 관리 통합**

React 상태 관리 라이브러리와 자연스럽게 연동된다.
...
`@react-three/drei`가 제공하는 유틸리티로 보일러플레이트를 줄인다:
...
Redux라면 action type, action creator, reducer, selector, provider 설정이 필요하다.

**2. React 외부에서 접근 가능**

`useFrame` 같은 Three.js 렌더 루프에서는 React 훅을 직접 호출할 수 없다. Zustand는 `getState()`로 훅 없이 상태에 접근 가능하다.
