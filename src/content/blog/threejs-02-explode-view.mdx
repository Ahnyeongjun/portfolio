...
description: "3D 모델의 각 파츠를 중심에서 바깥으로 펼쳐서 내부 구조를 보여주는 분해 뷰 기능을 구현한다."
date: "2025-02-05"
category: "Three.js"
...
3D 모델의 각 파츠를 중심에서 바깥으로 펼쳐서 내부 구조를 보여주는 기능이다.
...
R3F의 `useFrame`은 매 프레임마다 실행된다. 여기서 위치를 업데이트한다.
...
`Vector3.lerp(target, alpha)`는 현재 위치에서 목표까지 `alpha` 비율만큼 이동한다.
...
처음엔 빠르게, 목표에 가까워질수록 느려지는 자연스러운 easing 효과를 준다.
...
**단점**: 슬라이더로 실시간 제어하려면 매번 tween을 취소하고 다시 생성해야 한다
...
슬라이더 외에 Shift + 마우스 휠로도 분해 정도를 조절할 수 있다.
...
현재는 "중심에서 바깥으로" 단순 방식이다. 실제 조립 순서를 반영하면 더 직관적이다.
...
특정 부품은 위로, 특정 부품은 옆으로 분해되도록 방향을 지정할 수 있다.
