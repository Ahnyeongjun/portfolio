...
description: "React Three Fiber 프로젝트에서 Zustand를 사용한 상태 관리 패턴을 정리한다."
date: "2025-02-05"
category: "Three.js"
...
React Three Fiber 프로젝트에서 Zustand를 사용한 상태 관리 패턴을 정리한다.
...
`useFrame`은 매 프레임 실행되므로, 훅 호출이 아닌 `getState()`를 사용한다.
...
`useFrame` 콜백은 매 프레임 실행되지만, React 렌더 사이클과 별개다. 훅 호출 규칙을 위반하지 않으려면 `getState()`로 직접 접근한다.

### 패턴 3: 선택적 구독

전체 스토어가 아닌 필요한 값만 구독한다.
...
Zustand는 React 외부에서도 상태에 접근할 수 있다.
...
Redux DevTools에서 상태 변화를 확인할 수 있다.
