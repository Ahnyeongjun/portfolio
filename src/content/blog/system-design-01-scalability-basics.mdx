...
description: "책 1~3장(규모 확장성, 개략적 추정, 면접 공략법)을 요약 대신 Docker Compose로 단계별로 직접 쌓아본다. DNS·SQL/NoSQL 선택부터 로드밸런서, DB 복제, 캐시, 무상태 웹 계층, 샤딩까지, 각 단계마다 k6로 처리량을 실측해 책의 주장을 검증하는 절차."
date: "2026-07-26"
...
```
[단말기] → [DNS] → [로드밸런서] → [웹 서버 (다중화)] → [DB (마스터-슬레이브 복제)]
                                        ↕
                                     [캐시]
```
...
4. **DB 다중화(복제)**: 읽기/쓰기 분리 - 읽기가 압도적으로 많은 워크로드 대응.
5. **캐시 추가**: DB 부하 자체를 줄임.
6. **CDN**: 정적 자산을 사용자와 가까운 곳에서 서빙.
7. **무상태 웹 계층**: 웹 서버가 세션 상태를 들고 있지 않게 만들어 수평 확장을 쉽게 함.
8. **데이터 센터 다중화, 메시지 큐, 로그/메트릭/자동화**: 더 큰 규모·가용성을 위한 확장.
9. **데이터베이스의 수평적 확장(샤딩)**: DB 자체도 여러 대로 쪼갬.

각 단계는 직전 단계의 구체적인 한계를 해소하기 위해 추가된다 - 3단계(로드밸런서)는 앱 서버 1대가 죽으면 서비스 전체가 죽는 문제를, 4단계(DB 복제)는 마스터 DB 하나가 모든 읽기까지 떠안는 병목을, 5단계(캐시)는 매 요청마다 DB까지 가는 낭비를 해소한다. 8단계(메시지 큐·로그/메트릭 자동화)는 이미 DevOps 학습 로드맵 시리즈(RabbitMQ 비동기 파이프라인, Prometheus/Grafana 관측 스택)에서 별도로 깊게 다뤘으므로 이 글에서는 반복하지 않고 개념만 짚는다.
...
### 캐시 - 읽기 주도형(read-through) 전략과 고려사항
...
**CDN(Content Delivery Network)**은 지리적으로 분산된 서버의 네트워크로, 콘텐츠를 캐시해 사용자에게 더 가까운 곳에서 응답한다. **동적 콘텐츠 캐싱**은 요청마다 결과가 달라져서 상대적으로 어렵다 - 이 실습에서는 다루지 않는다. **정적 콘텐츠 캐싱**은 쿠키·요청 헤더 등의 정보에 기반해 이미지·CSS·HTML 같은 정적 자산을 캐시하는 것이다. 클라이언트에서 CDN까지의 거리가 클라이언트에서 원본 서버까지의 거리보다 훨씬 가깝다는 게 핵심이다.
...
### 데이터 센터, 메시지 큐, 로그/메트릭/자동화

여기서부터는 더 큰 규모를 향한 확장이다 - 데이터 센터를 여러 지역에 두어 재해에 대비하고, 메시지 큐로 서비스 간 결합을 느슨하게 만들고(→ DevOps 로드맵 시리즈에서 RabbitMQ로 이미 구현), 로그/메트릭/자동화로 무엇이 어디서 잘못됐는지 파악한다(→ DevOps 로드맵 시리즈에서 Prometheus/Grafana로 이미 구현). 이미 다른 시리즈에서 손으로 만들어본 부분이라 이 글에서는 개념만 짚고 넘어간다.
...
로그인 세션을 흉내내는 엔드포인트(`/session`)를 추가해, 상태 유지형과 무상태의 차이를 직접 재현한다.
...
### 7단계 (선택): CDN - AWS CloudFront로 실제 구현

로컬 Docker로는 "지리적으로 먼 원본 서버"라는 CDN의 핵심 조건 자체를 재현할 수 없다 - 그래서 이 단계만 실제 AWS에 만든다. 정적 자산(이미지 한 장이면 충분하다)을 S3에 올리고, 그 앞에 CloudFront를 붙인다.
...
- [ ] (선택) CDN: S3+CloudFront 실제 구성, origin 대비 응답시간·캐시 히트(X-Cache) 확인, 무효화(invalidation) 후 갱신 확인
- [ ] 전 단계 처리량·응답시간 변화를 표 하나로 정리
