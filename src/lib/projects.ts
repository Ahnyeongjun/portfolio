...
export const projects: Project[] = [
  ...
  },
  {
    ...
    longDescription: "위성영상을 수집·판독해 정식 보고서로 만들어내는 국가보안기관 대상 시스템입니다. 분석관은 CesiumJS 3D globe 위에서 위성영상을 보며 표적에 화살표·텍스트로 주석을 달아 판독보고서를 작성하고, 이를 정부 표준 문서(HWP)로 내려받습니다. 이 화면들의 프론트엔드와 API를 2년간 담당하며 end-to-end로 기능을 구현했습니다.",
    longDescriptionEn: "A national security agency system that collects and interprets satellite imagery to produce formal reports. Analysts view imagery on a CesiumJS 3D globe, annotate targets with arrows and text to author interpretation reports, and download the result as Korea's government-standard HWP document. Owned the frontend and API for these screens end-to-end over 2 years.",
    details: [],
    roleDetails: [],
    achievements: [
      "[수집 이력·통계] 위성영상 수집 현황을 확인할 화면이 없어 DB를 직접 조회해야 함 → 수집 이력·통계 조회 화면을 라우팅부터 그리드까지 신규 구축",
      "[모드 관리] 일반·스와이프·시계열분석·변화탐지 등 여러 모드가 공존하는 화면에서 공통 컴포넌트가 모드를 일관되게 인식하지 못해 동작이 꼬임 → getMode() 전역 조회와 컴포넌트별 모드 폴백·분기 로직으로 해결",
      "[판독보고서] 각 페이지의 3D globe 화면 캡처가 그대로 HWP 배경으로 삽입되는 구조라 화면 상태 제어가 중요 → CesiumJS 카메라 rect 계산 기반 뷰 북마크·복귀 기능 구현",
      "[컴포넌트 매니저] 팝업·iframe·인페이지 컴포넌트마다 통신 방식이 달라 호출 코드가 제각각 → execute() 하나로 위치와 무관하게 호출 가능한 공통 매니저 활용·개선",
    ],
    achievementsEn: [
      "[Collection History/Stats] No screen existed to check imagery collection status, requiring direct DB queries → built collection history/statistics screens from routing to grid",
      "[Mode Management] Shared components couldn't consistently tell which mode (normal/swipe/time-series/change-detection) was active, breaking behavior → resolved via a global getMode() query plus per-component mode fallback/branching",
      "[Interpretation Report] Each report page embeds a captured 3D globe screenshot directly as its HWP background, making view-state control critical → implemented camera-rectangle-based view bookmarking and restore in CesiumJS",
      "[Component Manager] Popups, iframes, and in-page components each communicated differently, fragmenting call code → leveraged and improved a shared manager exposing one execute() call regardless of component location",
