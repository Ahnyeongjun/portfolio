...
export const projects: Project[] = [
  ...
  },
  {
    ...
    roleDetails: [],
    achievements: [
      "[노드 자원 분배] K8s 기본 스케줄러가 노드별 메모리 사용률을 실시간 반영 못해 특정 노드에 AI 워크로드가 몰려 OOM 발생 → SaltStack Minion으로 노드 연결 상태·메모리 사용률 실시간 점검, 메모리 50% 미만 노드에만 AI 워크로드 자동 할당 - 노드 과부하·OOM 사전 차단",
