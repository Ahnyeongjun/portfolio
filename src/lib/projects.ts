...
export const projects: Project[] = [
  ...
  },
  {
    ...
    achievements: [
      ...
      "[가시화 전면 실패 장애] 가시화 작업이 전부 'No space left on device'로 실패했지만 마운트 용량은 충분 → 마운트 경로 직접 쓰기 테스트로 컨테이너가 아닌 스토리지 자체 문제로 특정, 스토리지 장비 NVMe 상태를 직접 확인하고 업체 운영 코드까지 추적 - 펌웨어 업그레이드 시 NVMe→system pool 데이터 이동 로직 누락이 근본 원인, 임시 패치로 즉시 복구 후 업체 정식 패치 연동",
      "[대용량 영상 OOM] 수억 픽셀 규모 위성영상을 통째로 메모리에 올리다 컨테이너가 OOM으로 강제 종료 → 픽셀 수 기준 일반·대용량 처리 경로를 분기, 대용량 영상은 GDAL 파이프라인으로 먼저 축소한 뒤 폴리곤화 - 자원이 부족한 환경에서도 구조적으로 작업이 진행되도록 개선",
    ...
    achievementsEn: [
      ...
      "[Total Visualization Outage] All visualization jobs failed with 'No space left on device' despite the mount showing free capacity → isolated the fault to the storage device itself (not the container) via a direct write test on the mount path, then traced through the vendor's operating code after checking NVMe status directly - root cause was a missing NVMe→system-pool data migration step introduced by a firmware upgrade; restored service immediately with a hotfix, then integrated the vendor's official patch",
      "[Large-Image OOM] Loading multi-hundred-megapixel satellite imagery entirely into memory force-killed containers with OOM → branched processing into standard/large-image paths by pixel count, downsampling large images via a GDAL pipeline before polygonization - kept processing structurally viable even under constrained resources",
