...
export const projects: Project[] = [
  ...
  },
  {
    ...
    description: "분석관이 3D globe 위에서 위성영상을 보며 표적을 판독하고, 정부 표준 문서(HWP)로 보고서를 산출하는 국가보안기관 대상 시스템의 프론트엔드·API를 2년간 개발.",
    descriptionEn: "Frontend and API development, over 2 years, for a national security agency system where analysts interpret satellite imagery on a 3D globe and produce formal reports in Korea's government-standard HWP document format.",
    ...
    longDescription: "위성영상을 수집·판독해 정식 보고서로 만들어내는 국가보안기관 대상 시스템입니다. 분석관은 CesiumJS 3D globe 위에서 위성영상을 보며 표적에 화살표·도형·텍스트·이미지로 주석을 달아 판독보고서를 작성하고, 이를 정부 표준 문서(HWP)로 내려받습니다. 이 화면들의 프론트엔드와 API를 2년간 담당하며 end-to-end로 기능을 구현했습니다.",
    longDescriptionEn: "A national security agency system that collects and interprets satellite imagery to produce formal reports. Analysts view imagery on a CesiumJS 3D globe, annotate targets with arrows, shapes, text, and images to author interpretation reports, and download the result as Korea's government-standard HWP document. Owned the frontend and API for these screens end-to-end over 2 years.",
    details: [],
    roleDetails: [],
    achievements: [
      ...
      "[MyBatis 코드 생성] 테이블 수가 많아(40여 개) 신규 테이블마다 Entity·MyBatis XML mapper를 손으로 작성해야 함 → DB information_schema를 조회해 컬럼 타입·PK·주석 기반으로 Entity(Entity/ReqData/Result/Search)와 CRUD·검색 XML mapper를 자동 생성하는 도구를 팀 내부에 구축",
    ],
    achievementsEn: [
      ...
      "[MyBatis Code Generation] Manually writing an Entity class and MyBatis XML mapper for every new table (of ~40) didn't scale → built an internal tool that queries the DB's information_schema (column types, PK, comments) to auto-generate Entity classes (Entity/ReqData/Result/Search) and CRUD/search XML mappers",
    ],
  },
  {
    id: "team-mcp-agent",
