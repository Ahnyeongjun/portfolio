...
export const PROFILE = {
    ...
    groups: [
      {
        ...
        items: [
          ...
          "Redis 역색인 기반 세션 통제·로그인 시도 차단(TTL 자동 만료)",
          ...
          "신규 플랫폼 DB 접근 계층을 Go API로 중앙화 - 스키마 변경 영향 범위 축소",
        ],
      },
      {
        ...
        items: [
          ...
          "Spring AOP + MyBatis Executor 인터셉터 Outbox 라이브러리 직접 개발 - CDC 의존 제거, 이벤트 유실 0건",
  ...
  projects: [
    ...
    },
    {
      ...
      blocks: [
        {
          label: "인증·인가 계층 설계 (Spring Security + AOP)",
          situation: "엔드포인트가 늘수록 컨트롤러마다 권한 체크 코드가 흩어져 누락·중복 위험이 커짐",
          cause: "인증과 인가 로직이 분리되지 않고 각 컨트롤러에 개별 구현되어 있어, 신규 API마다 권한 체크를 매번 새로 작성해야 했음",
          actions: [
            "인증은 Spring Security 필터 체인에서 일괄 처리",
            "인가는 커스텀 어노테이션 + AOP로 선언적으로 분리 - 신규 API는 어노테이션만 붙이면 권한 검증 적용",
          ],
          result: "인가 로직 중앙화로 권한 적용 누락 구조적 방지, 비즈니스 코드와 보안 코드 분리",
          brief: [
            "엔드포인트가 늘수록 컨트롤러마다 권한 체크 코드가 흩어져 누락·중복 위험이 커지고 있었습니다.",
            "인증은 Spring Security 필터 체인에서 일괄 처리하고, 인가는 커스텀 어노테이션+AOP로 선언적으로 분리해 신규 API는 어노테이션만 붙이면 되도록 만들었습니다.",
          ],
        },
        {
          label: "Redis 기반 인증·세션 관리",
          situation: "사용자별 동시 로그인 통제와 로그인 시도 공격 방어가 필요했으나, 토큰 단방향 저장만으로는 '이 사용자가 몇 세션 접속 중인가'를 답할 수 없었음",
          cause: "토큰 검증만 가능하고 사용자 기준 역조회가 불가능한 저장 구조였음",
          actions: [
            "토큰과 함께 user→token 역색인을 Redis에 저장해 사용자별 활성 세션을 즉시 조회·통제",
            "로그인 실패를 IP·계정 단위로 카운트해 5회 초과 시 차단 - 차단 해제는 TTL 자동 만료로 처리",
          ],
          result: "세션 상태를 앱 메모리가 아닌 Redis로 외부화해 서버 증설과 무관하게 동작하는 구조, 해제 배치 등 운영 코드 0",
          brief: [
            "사용자별 동시 로그인 통제와 로그인 시도 공격 방어가 필요했지만, 토큰 단방향 저장만으로는 활성 세션 수를 답할 수 없었습니다.",
            "user→token 역색인을 Redis에 저장해 세션을 즉시 조회·통제하고, 로그인 실패를 IP·계정 단위로 카운트해 5회 초과 시 TTL 자동 만료로 차단했습니다.",
          ],
        },
        {
          label: "다운로드 엔드포인트 통합",
        ...
        {
          label: "신규 플랫폼 DB 접근 계층 중앙화",
          situation: "서비스별 ORM 직접 접근으로 DB 로직·자격증명이 여러 서비스에 흩어져 있었음",
          cause: "스키마가 바뀔 때마다 여러 서비스를 동시에 수정해야 했음",
          actions: [
            "DB 접근을 Go 기반 API 한 곳으로 중앙화",
          ],
          result: "스키마 변경 영향 범위를 API 레이어로 축소, 언어 무관 동일 접근 방식 확보",
          brief: [
            "서비스별 ORM 직접 접근으로 DB 로직·자격증명이 흩어져 있어 스키마 변경 시 여러 서비스를 동시에 수정해야 했습니다.",
            "신규 플랫폼에서 DB 접근을 Go 기반 API 한 곳으로 중앙화해 스키마 변경 영향 범위를 API 레이어로 좁혔습니다.",
          ],
        },
      ],
    },
