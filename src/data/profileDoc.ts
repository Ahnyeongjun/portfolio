    ...
    },
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          situation: "에어갭 환경에서 DB 양방향 동기화·분산 ID 발급이 필요했고, 샤딩 가능성도 대비해야 했음. CDC는 slot 파손·멱등키 결함·bytea 타입 불일치·EWKB 파싱 실패가 겹쳐 있었음",
          cause: "외래키는 샤딩 시 걸림돌이지만 관측성 데이터는 정합성 손실이 치명적이지 않음(단, 유저 생성 같은 경로엔 이 판단이 잘못 적용돼 버그 발생). Debezium은 slot 파손 시 전체 재동기화가 필요했고, 멱등키는 타임스탬프 포함 서명이라 재전달마다 값이 바뀌었으며, bytea·EWKB는 각각 타입 선언과 파싱 방식이 잘못돼 있었음",
          actions: [
            "정합성 요구가 낮은 테이블만 외래키 제외, 유저 생성은 트랜잭션 경계로 원자성 재확보",
            "폐쇄망용 Outbox+Snowflake 직접 구현, 유지 구간은 상태 판정 재정렬+자가치유로 자동 복구",
            "휘발성 컬럼 제외 정규화 서명으로 멱등키 재설계, bytea는 LargeBinary·EWKB는 전용 디코더로 재정리",
          ],
          result: "샤딩 여지 확보, 이벤트 유실 0건, CDC dedup 안전 처리, 이진 데이터·지오메트리 파싱 정합성 확보",
          brief: [
