      ...
      blocks: [
        {
          ...
          brief: [
            ...
            "보안 체크리스트·JUnit 통합 테스트를 선반영하고 SQL injection을 화이트리스트 검증으로 교체했습니다. 조건부 PostGIS 실행·페이지네이션·BLOB 분리·인덱스·Redis 캐싱·트랜잭션 통합으로 병목을 해소했고, 헬스체크도 프로세스 생존 확인만으로 경량화해 파드 안정성을 함께 확보했습니다.",
          ],
        },
        {
          ...
          result: "테이블 성장에 따른 샤딩 여지를 미리 확보하면서, 정합성이 실제로 중요한 경로의 버그도 근본 해결",
          brief: [
            "위성 메타·추론 결과 테이블의 로우가 계속 늘어 향후 샤딩을 염두에 둬야 했는데, 이 판단이 유저 생성처럼 정합성이 중요한 경로에도 그대로 적용돼 유저·권한 insert가 각각 별도 트랜잭션으로 실행되며 정합성이 깨지는 버그가 있었습니다.",
            "정합성 요구가 낮은 테이블에 한해 외래키를 두지 않는 스키마로 설계하고, 유저 생성처럼 정말 필요한 경로는 하나의 트랜잭션으로 묶어 원자성을 확보했습니다.",
          ],
        },
        {
          ...
          result: "이벤트 유실 없이 안정적으로 운영, 외부 코디네이터 없이 전역 유일 ID 발급·망 추적 확보. 23개 테이블의 CDC 반영을 단일 dedup 로직으로 안전 처리",
          brief: [
            "외부망↔폐쇄망이 물리 분리된 환경에서 Debezium CDC의 replication slot이 반복 파손됐고, 재전달된 CDC 이벤트가 멱등키 설계 결함(타임스탬프 포함 서명)으로 중복 반영되는 문제도 있었습니다.",
            "폐쇄망 동기화용은 자체 Outbox 라이브러리+Snowflake로 Debezium 의존을 걷어냈고, 유지되는 CDC 파이프라인은 자가치유 스크립트로, 이벤트 중복은 휘발성 컬럼 제외 멱등키 재설계로 각각 안정화했습니다.",
          ],
        },
        {
          label: "영상 서빙 속도 개선",
      ...
      blocks: [
        {
          ...
          result: "인가 로직 중앙화로 권한 적용 누락 구조적 방지, 세션 상태를 앱 메모리가 아닌 Redis로 외부화해 서버 증설과 무관하게 동작하는 구조. 위험도별 요청 제한과 CSP·SQLi 보안 지적사항 해소",
          brief: [
            "엔드포인트가 늘수록 컨트롤러마다 권한 체크 코드가 흩어져 있었고, 고위험 엔드포인트도 무제한 호출이 가능했으며, 외부 보안 진단에서 CSP 미비·SQL Injection 등 10건 이상을 지적받았습니다.",
            "인증·인가를 필터+AOP로 선언적으로 분리하고 Redis 기반 세션·Rate Limiting을 구축했으며, CSP는 런타임 동적 생성으로, SQL Injection은 13개 매퍼 파라미터 바인딩 전환으로 해소했습니다.",
          ],
        },
        {
          label: "다운로드 엔드포인트 통합",
        ...
        },
        {
          ...
          result: "스키마 변경 영향 범위를 API 레이어로 축소, 언어 무관 동일 접근 방식 확보. 무제한 병렬화의 위험성을 인지하고 세마포어 기반 제어로 전환하는 트레이드오프 판단 확보",
          brief: [
            "서비스별 ORM 직접 접근으로 DB 로직·자격증명이 흩어져 있었고, 다중 레이어 위성영상 타일 서버는 무제한 goroutine 병렬화 도입 후 두 차례나 원복하는 시행착오를 겪었습니다.",
            "DB 접근을 Go API로 중앙화하고, 타일 서버는 CPU 코어 기반 동적 세마포어 + 타임아웃 + TTL 캐시로 안전한 병렬 처리 구조를 정착시켰습니다.",
          ],
        },
      ],
    },
