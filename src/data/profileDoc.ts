      ...
      blocks: [
        {
          ...
          result: "k6로 로그인 세션 풀림·특정 API/로직 지연 재현(에러율 11.22%→0%, 처리량 392→1,177 req/s), 유닛테스트로 생성·수정이 다른 트랜잭션에서 이뤄져 고아 데이터가 발생하는 문제와 동기화 로직의 레이스 컨디션 방치 문제를 다수 발견·수정",
          brief: [
            "테스트는 sh 스크립트뿐이라 문제가 생길 때마다 추가될 뿐 미래에 대한 대처가 어려웠고, 동시 부하 상황의 문제를 재현할 방법이 없었습니다.",
            "가상의 유저를 상정하고 플로우를 정한 뒤 k6로 부하테스트를 진행하며 세션 문제와 특정 API/로직 지연을 잡아냈고, 유닛테스트 작성 과정에서 트랜잭션 범위 문제도 발견해 수정했습니다.",
          ],
        },
        {
