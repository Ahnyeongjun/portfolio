...
export const PROFILE_PLATFORM = {
  ...
  tagline: "다양한 언어와 환경을 다뤄온 5년차 인프라 엔지니어입니다. 에어갭 환경부터 대국민 서비스, 자사 서비스까지 온프레미스에서 K8s로 직접 구축하며 레거시를 끊임없이 고도화해왔습니다.",
  summary: [
    {
      head: "다양한 기관의 인프라를 매번 새로 설계했습니다",
      body: "한국항공우주연구원(KARI)·국가보안기관·NIPA 등 인터넷 접근성부터 규모까지 서로 다른 기관을 대상으로 인프라를 매번 새로 설계했습니다. 온프레미스 클러스터 구축, 에어갭 환경 운영, MSA 전환에 따른 네트워킹·시크릿·배포·관측성 고도화까지 인프라 전 영역을 담당했습니다.",
    },
    {
      head: "서비스가 커지는 만큼 인프라도 함께 성장시켰습니다",
      body: "처음에는 서버에 직접 배포하는 방식이었지만, 점차 배포를 자동화하고 프로그램을 컨테이너 단위로 표준화해 어디서든 동일하게 실행되도록 만들었습니다. 서버 규모가 커지면서는 여러 대의 서버를 자동으로 관리해주는 쿠버네티스를 도입해, 장애가 나도 스스로 복구되고 상태를 실시간으로 들여다볼 수 있는 환경을 갖췄습니다. 서비스가 여러 개로 쪼개지면서(MSA)는 서비스끼리 안전하게 데이터를 주고받는 방식을 도입했고, 비밀번호·인증 정보 같은 민감한 값도 사람이 직접 다루지 않고 자동으로 안전하게 관리되도록 만들었습니다. 이처럼 서비스를 직접 키워오면서 저 역시 함께 성장해왔습니다.",
    },
    {
      head: "반복되는 작업은 자동화로 걷어냅니다",
      body: "CRUD 코드를 매번 손으로 짜는 대신 DB 스키마 기반 자동 생성 도구를 만든 것을 시작으로, 작업 이력 정리·일정 연동·업무 보고까지 대신해주는 사내 업무 자동화 에이전트를 직접 만들어 썼습니다. 최근에는 팀 공용 작업지시서(CLAUDE.md)로 Jira·Bitbucket을 연동해 브랜치를 자동으로 나누고 작업 이력을 관리하도록 만들어, 브랜치별로 AI가 개발을 진행하면 저는 작업 이력과 PR로 최종 확인만 하는 방식으로 일하고 있습니다.",
    },
  ],
      ...
      blocks: [
        {
          ...
          brief: [
            ...
            "팀원과 함께 스토리지 장비에 직접 접속해 NVMe 실사용량을 확인하고 업체 코드까지 검토해 펌웨어 이관 로직 누락을 찾아, 임시 패치 후 정식 패치까지 연동해 복구했습니다. 외부 레퍼런스 없이 하루 이상 걸리던 원인 파악·수정을 수 시간 이내로 단축했습니다.",
          ],
        },
        {
        ...
        },
        {
          ...
          result: "베어메탈에서 운영 가능한 시스템까지 인프라 전 과정에 핵심 참여한 경험 확보",
          brief: [
