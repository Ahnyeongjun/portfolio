  ...
  summary: [
    {
      ...
      body: "국가기관 납품 플랫폼들에서 k6 부하테스트 기반 성능 개선을 주도해 50VU 동시 요청 에러율을 11.22%에서 0%로, 처리량을 392에서 1,177 req/s로 끌어올렸습니다. 병목의 대부분은 DB에 있었고, 조건부 PostGIS 실행·페이지네이션·BLOB 분리·Redis 캐싱으로 위성 메타 목록 조회를 38초에서 159ms로 단축했습니다. 쿼리 한 줄이 서비스 전체의 안정성을 좌우한다는 것을 실무에서 체득했습니다.",
    },
    {
...
export const PROFILE_PLATFORM = {
  ...
  summary: [
    {
      ...
      body: "한국항공우주연구원(KARI)·국가보안기관·NIPA 등 인터넷 접근성부터 규모까지 서로 다른 기관을 대상으로 인프라를 매번 새로 설계했습니다. 온프레미스 클러스터 구축, 에어갭 환경의 운영, MSA 전환에 따른 네트워킹·시크릿·배포·관측성 고도화까지 인프라 전 영역을 담당했습니다.",
    },
    {
      head: "서비스가 커지는 만큼 인프라도 함께 성장시켰습니다",
      body: "처음에는 서버에 직접 배포하는 방식이었지만, 점차 배포를 자동화하고 프로그램을 컨테이너 단위로 표준화해 어디서든 동일하게 실행되도록 만들었습니다. 서버가 여러 대로 늘어 수동 관리가 한계에 닿자 쿠버네티스를 도입했고, 장애가 나도 스스로 복구되고 상태를 실시간으로 들여다볼 수 있는 환경을 갖췄습니다. 서비스가 여러 개로 쪼개진(MSA) 뒤에는 서비스 간 데이터가 중간에 유실되는 문제가 생겨 안전하게 주고받는 구조를 도입했고, 민감한 인증 정보도 사람 손을 거치지 않게 자동화했습니다. 이처럼 서비스를 직접 키워오면서 저 역시 함께 성장해왔습니다.",
    },
    {
      head: "반복되는 작업은 자동화로 걷어냅니다",
      body: "CRUD 코드를 매번 손으로 짜는 대신 DB 스키마 기반 자동 생성 도구를 만든 것을 시작으로, 작업 이력 정리·일정 연동·업무 보고까지 대신해주는 사내 업무 자동화 에이전트를 직접 만들어 썼습니다. 최근에는 팀 공용 작업지시서(CLAUDE.md)와 Jira·Bitbucket 연동으로 AI 에이전트 개발 워크플로우를 설계해, 브랜치 단위 작업 분배부터 이력 관리·PR 리뷰까지 이어지는 체계를 팀에 정착시켰습니다.",
    },
  ],
  ...
  career: {
    ...
    metrics: ["kubeadm 베어메탈 클러스터 구축", "GPU 4장 70파드 병렬 추론", "처리 워커 1→15개 수평 확장", "에어갭 환경 롤링 배포 운영", "이벤트 유실 0건"],
    groups: [
      {
        title: "프로젝트 개요",
        items: [
          { text: "NIPA 위성 변화탐지 플랫폼 - 클러스터·메시징 인프라 고도화 · 주도 및 AI 모델 개발(팀 5명)" },
          { text: "KARI 위성영상 AI 처리 플랫폼 - 클러스터·비동기 파이프라인 구축 · 주도 및 풀스택 개발(팀 3명)" },
          { text: "국가보안기관 위성영상 플랫폼 - 에어갭 인프라 운영·신규 구축 · 핵심 참여 및 풀스택 개발(팀 6명)" },
        ],
      },
      {
        title: "클러스터 구축·운영",
        items: [
          { text: "컨테이너 장애 시 작업이 유실되고 GPU가 유휴 상태였던 추론 환경을 kubeadm 베어메탈 클러스터+GPUShare fraction 분할로 재구성 - GPU 4장 70파드 병렬 추론 (KARI)" },
          { text: "에어갭(인터넷 완전 차단) 환경에서 수십 대 서버 규모 클러스터를 롤링 방식으로 서비스 중단 없이 운영, Zabbix 커스텀 대시보드 모니터링으로 장애 사전 감지 (국가보안기관)" },
          { text: "반입 도구가 제한된 환경에서 클러스터 프로비저닝 쉘 스크립트화 - 수동 구축 1~2일 → 2~3시간 (국가보안기관)" },
          { text: "K8s 인증서 만료 대응 자동화 - 연 1회 현장 방문 작업 제거 (국가보안기관)" },
          { text: "로컬·개발·에어갭 3환경 매니페스트 분리 관리 - 환경별 오배포 방지 (국가보안기관)" },
        ],
      },
      {
        title: "CI/CD·배포",
        items: [
          { text: "Jenkins 명령형 빌드·배포를 Argo Workflows CI + ArgoCD GitOps 배포로 분리 - 배포 드리프트 추적성 확보 (NIPA)" },
          { text: "배포 후에만 드러나던 부하·트랜잭션 결함을 k6 부하테스트·유닛테스트의 CI 편입으로 배포 전 검출 - 에러율 11.22%→0% (KARI)" },
        ],
      },
      {
        title: "관측성·장애 대응",
        items: [
          { text: "MSA 전환으로 서비스별로 흩어진 로그·트레이스를 OTel 기반 관측 스택(Prometheus·Tempo·Grafana) 신규 구축으로 통합 - 장애 원인분석 1~2시간→30분 이내 (NIPA)" },
          { text: "표면 에러(디스크 부족)를 컨테이너→마운트→디바이스→펌웨어로 추적해 NVMe 펌웨어 이관 로직 누락을 발견·복구 - 하루 이상 걸리던 원인 파악·수정을 수 시간 이내로 단축 (국가보안기관)" },
        ],
      },
      {
        title: "네트워킹·보안",
        items: [
          { text: "서비스가 각자 노출돼 보안 정책이 분산되던 구조를 Nginx 게이트웨이로 통합 - 서비스 7개의 HTTPS 종단·인증서·보안 헤더 1곳 관리, Wireshark 정보 노출 점검 (KARI)" },
      ...
      },
      {
        title: "메시징·데이터 파이프라인",
        items: [
          { text: "DB 폴링 락 경합으로 워커를 늘려도 확장되지 않던 처리 구조를 RabbitMQ 단계별 큐(ack/nack+DLQ)로 전환 - 처리 워커 1개→15개, 작업 유실 0건 (NIPA)" },
          { text: "에어갭 망연계에서 Debezium slot이 반복 파손되던 문제를 AOP+MyBatis Outbox 라이브러리 직접 개발로 해결 - CDC 의존 제거, 이벤트 유실 0건 (KARI)" },
          { text: "스키마 변경마다 7개 서비스를 동시 수정해야 했던 DB 직접 접근 구조를 Go API 1곳으로 중앙화 - 변경 영향 범위를 API 레이어로 축소 (국가보안기관)" },
        ],
      },
      {
        title: "업무 자동화",
        items: [
          ...
          { text: "Claude 스킬·훅 기반 에이전트 개발 - 작업 계획·평가 자동화" },
          { text: "Jira·Bitbucket 연동 자동화 - 자동 브랜치 생성 → PR 문화 정착" },
          { text: "Cilium·kube-vip·ArgoCD·OTel 등 신규 인프라 스택 사내 세미나 발표 - 우수성 입증해 도입 설득 (NIPA)" },
        ],
      },
    ] as CareerGroup[],
  },
      ...
      blocks: [
        {
          ...
          result: "컨테이너→마운트→디바이스→펌웨어로 경계를 넘어 근본 원인 추적, 외부 레퍼런스 없이 하루 이상 걸리던 원인 파악·수정을 수 시간 이내로 단축",
          brief: [
        ...
        },
        {
          ...
          result: "물리 서버부터 운영 가능한 시스템까지 전 과정 수행 경험 확보",
          brief: [
            "관리형 인프라 없이 물리 서버 설치부터 시작해야 하는 신규 구축 프로젝트였습니다.",
            "물리 서버·K8s 클러스터 구성부터 수집기 통합·포맷 변환 자동화·히스토그램 스트레칭·자동 삭제 워크플로우 구현까지 인프라 전 과정에 핵심 참여했습니다.",
          ],
        },
      ],
    },
