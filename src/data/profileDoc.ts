  ...
  projects: [
    {
      ...
      blocks: [
        ...
        {
          label: "위성영상 메타 조회 성능 개선 - 컬럼 프로젝션·페이지네이션 도입",
          situation: "위성영상 메타데이터 조회 API가 ORM 풀 엔티티 하이드레이션 후 재직렬화하는 구조였고, 이미 4326 SRID로 저장된 컬럼에 불필요한 ST_SetSRID를 매번 재적용, 페이지네이션도 없어 대량 결과를 한 번에 반환",
          cause: "필요 없는 컬럼까지 select하고 반복 파싱되는 datetime 값도 매 행마다 재계산해 응답이 느려졌고, 응답 크기도 예측할 수 없었음",
          actions: [
            "필요한 컬럼만 select하는 컬럼 프로젝션으로 전환, 불필요한 ST_SetSRID 제거",
            "반복 파싱되던 datetime 값을 루프 밖에서 1회만 계산",
            "필터 필드를 모델에 실제 존재하는 컬럼만 허용하는 화이트리스트 방식으로 검증(SQL Injection 표면 차단 겸용), page/limit 기반 페이지네이션과 총건수 API 추가",
          ],
          result: "fetch·serialize 단계별 타이밍 계측 확보, 기본 페이지 크기 1000건으로 응답 크기 예측 가능",
          brief: [
            "위성영상 메타 조회 API가 ORM 풀 엔티티 하이드레이션과 중복 SRID 변환, 페이지네이션 부재로 대량 조회 시 응답이 느리고 크기도 예측 불가능했습니다.",
            "컬럼 프로젝션 전환, 중복 공간연산 제거, 화이트리스트 기반 동적 필터, page/limit 페이지네이션을 도입해 응답 속도와 크기를 통제 가능하게 만들었습니다.",
          ],
        },
        {
          label: "db-api 계층형 리팩토링 - models·services·api 3계층 분리",
          situation: "초기 db-api는 FastAPI 튜토리얼 구조를 그대로 사용해 테이블이 늘어날 때마다 models·crud·router 각 1개 파일에 로직이 계속 누적되는 구조",
          cause: "리소스별 관심사가 한 파일에 뒤섞여 변경 영향범위를 파악하기 어려웠고, 신규 리소스를 추가할 때마다 대형 파일을 더 키우는 방식으로만 확장되고 있었음",
          actions: [
            "테이블(엔티티) 단위로 models를, CRUD 로직을 services의 서비스 클래스로, HTTP 라우팅을 api의 라우터 모듈로 분리",
            "이 구조 위에서 이후 신규 리소스(즐겨찾기·페이지네이션 등)가 3계층 패턴을 그대로 따라 확장되도록 정착",
          ],
          result: "단일 파일 2~3개(약 1,000줄+) 구조를 계층별 다중 파일 구조로 전환, 이후 신규 기능이 이 패턴 위에서 반복 확장",
          brief: [
            "FastAPI 튜토리얼 구조를 그대로 유지한 db-api가 테이블 증가에 따라 models·crud·router 각 1개 파일에 로직이 누적돼 유지보수성이 떨어지고 있었습니다.",
            "엔티티 단위 models, 비즈니스 로직 담당 services, HTTP 계층 api로 3계층 분리 리팩토링을 수행해 이후 신규 기능들이 이 구조 위에서 확장되도록 만들었습니다.",
          ],
        },
      ],
    },
    {
      ...
      blocks: [
        ...
        {
          label: "CDC 이진 데이터 정합성 확보 - BLOB 타입 불일치·base64 디코딩 버그",
          situation: "공지 첨부파일·팝업 이미지 등 bytea 컬럼이 CDC 경로를 거치며 이미지가 깨지는 문제 발생",
          cause: "SQLAlchemy 모델에 bytea 컬럼이 String으로 잘못 선언돼 Postgres 원본과 타입이 불일치했고, 1차 수정도 실제 base64 디코딩 없이 문자열을 UTF-8 바이트로만 변환해 여전히 이미지가 깨졌음",
          actions: [
            "컬럼 타입을 LargeBinary로 재정의하고 base64.b64decode()로 정정",
            "Debezium 소스 설정에 binary.handling.mode=base64를 명시해 상류부터 인코딩 방식을 고정, base64 실패 시 hex 디코딩으로 폴백하는 방어 로직 추가",
          ],
          result: "bytea 컬럼 포함 테이블도 타입 선언→전송 인코딩→디코딩 3단 계층이 일관되게 처리되도록 정리, 이미지·첨부파일 CDC 동기화 정상화",
          brief: [
            "bytea 컬럼이 ORM에 String으로 잘못 선언되고 1차 수정도 잘못된 디코딩 방식이라 CDC로 동기화된 이미지·첨부파일이 계속 깨졌습니다.",
            "컬럼 타입을 LargeBinary로 재정의하고 Debezium 이진 처리 모드를 base64로 고정, base64 실패 시 hex 폴백까지 추가해 근본 원인을 제거했습니다.",
          ],
        },
        {
          label: "EWKB 지오메트리 파싱 안정화 - 수동 비트마스킹 제거",
          situation: "PostGIS EWKB를 일반 WKB 파서로 처리하며 SRID 유무를 바이트 오프셋·비트마스크로 직접 잘라내고 있어, 지오메트리 타입·엔디안 조합에 따라 파싱이 간헐적으로 실패",
          cause: "좌표 소수점 표현이 Go·Python 양쪽에서 통일돼 있지 않아, CDC 멱등키에 쓰이는 좌표 문자열 매칭까지 깨질 수 있는 구조였음",
          actions: [
            "Go는 go-geom의 wkb 디코더를 ewkb로 교체해 SRID 파싱을 라이브러리에 위임",
            "Python은 shapely.wkb + rounding_precision=16으로 좌표 포맷을 Go·Python 양쪽에서 통일",
          ],
          result: "수동 바이너리 파싱 코드 제거, 파싱 실패 재시도 분기 제거, 좌표 포맷 통일로 CDC 멱등키 매칭 안정성 확보",
          brief: [
            "EWKB를 일반 WKB 파서로 처리하며 SRID를 수동 비트마스킹으로 잘라내 간헐적으로 파싱이 실패했고, 좌표 소수점 표현도 Go·Python이 서로 달라 CDC 멱등키 매칭에 영향을 줄 수 있었습니다.",
            "EWKB 전용 디코더로 교체하고 좌표 소수점 자릿수를 16자리로 양쪽 통일해 파싱 실패와 멱등키 불일치 가능성을 함께 제거했습니다.",
          ],
        },
        {
          label: "영상 서빙 속도 개선",
      ...
      blocks: [
        ...
        },
        {
          ...
          actions: [
            ...
            "대용량 파일도 끊기지 않도록 스트리밍 응답 기반으로 재구성해 이어받기(재개 다운로드)를 지원",
          ],
          result: "파일 크기·전송 방식과 무관한 단일 인터페이스, 파일 깨짐 제거, 스트리밍 기반 이어받기 지원",
          brief: [
            "일반 다운로드와 대용량(INNORIX) 전송의 엔드포인트가 분리돼 있었고, DRM 재암호화가 요청마다 개별 실행돼 조각별 바이트 구조가 달라지며 파일이 깨지는 동시성 버그가 있었습니다.",
            "/download.do 단일 엔드포인트로 통합하고, 요청 직렬화·암호화 파일 재사용으로 DRM 동시성 버그를 수정했으며, 스트리밍 기반 이어받기를 지원하도록 재구성했습니다.",
          ],
        },
        {
          label: "공통 base 모듈 · mapper·entity 자동 생성 도구",
        ...
        {
          label: "공통 컴포넌트 매니저 - 팝업·iframe 간 postMessage 레지스트리 버그 수정",
          situation: "부모창-자식창(iframe·팝업)에 흩어진 지도·레이어트리·GIS 툴바 등 UI 컴포넌트를 postMessage로 등록·조회·원격 실행하는 레지스트리에서, 특정 컴포넌트 호출이 이유 없이 조용히 실패",
          cause: "동적 키 조회에 점 표기법(windws.winId)을 써서 항상 undefined를 반환하는 버그, 창이 이미 닫혔는지 확인하지 않아 닫힌 팝업을 살아있는 것으로 오인하는 null 체크 누락, 중복 요청 방지 로직이 컴포넌트 ID만으로 키를 잡아 같은 컴포넌트의 다른 함수 호출까지 통째로 막아버리는 과잉 억제가 겹쳐 있었음",
          actions: [
            "windws[winId] 브래킷 표기법으로 수정하고 창 생존 여부(win?.window) null 체크 추가",
            "중복 요청 방지 키를 compId 단독에서 compId+fnName 조합으로 세분화, 고빈도 호출 함수 2종은 억제 로직에서 예외 처리",
            "창 참조 정리(delete) 시점을 두고 '정리하면 재오픈 시 참조 소실, 정리 안 하면 iframe 재오픈 시 콜백 미실행'이라는 트레이드오프를 직접 확인하며 조정",
          ],
          result: "점/브래킷 표기법 버그와 null 체크 누락 버그 수정, 컴포넌트+함수 조합 키로 과잉 억제 제거",
          brief: [
            "팝업·iframe 간 UI 컴포넌트를 postMessage로 중계하는 레지스트리가 점 표기법 오표기로 항상 실패하거나, 닫힌 팝업을 살아있는 것으로 오인하거나, 같은 컴포넌트의 다른 함수 호출까지 막는 과잉 억제를 갖고 있었습니다.",
            "브래킷 표기법으로 수정하고 null 체크를 추가했으며, 억제 키를 컴포넌트+함수 조합으로 세분화하고 창 참조 정리 시점의 트레이드오프까지 확인해 안정화했습니다.",
          ],
        },
        {
          label: "에어갭 무중단 운영",
          ...
          actions: [
            ...
            "Zabbix 커스텀 대시보드로 장애 사전 감지 체계 구축, 에러 검색 서비스와 sh 스크립트 기반 에러 감지를 함께 구축해 외부 검색 없이도 장애 대응이 가능하도록 함",
          ],
          ...
          brief: [
            ...
            "스토리지→디바이스→펌웨어까지 추적해 NVMe 펌웨어 버그를 근본 원인으로 특정해 복구하고, 대용량 영상은 GDAL 축소 파이프라인으로, 장애는 Zabbix·에러 검색 서비스·sh 기반 감지로 사전 대응하도록 만들었습니다.",
          ],
        },
        {
