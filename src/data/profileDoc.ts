    ...
    groups: [
      {
        ...
        period: "2022.05 ~ 진행 중",
        items: [
          "프론트엔드 - CesiumJS 기반 판독보고서 저작 도구, 팝업·iframe 공통 컴포넌트 매니저 개발",
          "보안 체계 - Spring Security 인증·인가 설계, Redis 세션 통제, 위험도별 Rate Limiting, CSP·SQL Injection 등 외부 보안 진단 대응",
          "다운로드 처리 - 일반/대용량(INNORIX) 엔드포인트 통합, 스트리밍 UX 재설계",
          "내부 도구·데이터 관리 - MyBatis mapper·entity 자동 생성 도구, 표적 데이터 감사로그·계단식 삭제·정렬 로직",
          "외부 연동 - HWP 판독보고서 외부 시스템 연동 파이프라인",
          "운영 - 에어갭 환경 8대 서버 클러스터 무중단 운영, Zabbix 사전 감지, 클라우드 배포 환경 대응",
          "신규 구축 - DB 접근 계층 중앙화, 위성 이미지 ETL 로직, 타일 서버 동시성 설계, 베어메탈 인프라 구성",
        ],
      },
      {
        ...
        items: [
          "API 200여 개·테이블 35개 규모 시스템을 ETL 파이프라인을 제외한 전 모듈 혼자 설계·구현",
          "테스트 - 유닛테스트·k6 부하테스트 도입, API 성능 최적화",
          "DB 설계 - 외래키 없는 스키마 설계, 트랜잭션 경계 버그 수정",
          "데이터 동기화 - Outbox 라이브러리 직접 개발, CDC 이벤트 멱등키 설계, Debezium 자가치유 스크립트",
          "서버 개발 - Go 기반 영상 서빙 서버(WMS·WMTS·MVT), 대용량 산출물 다운로드 재설계",
          "AI 모델·추론 - 객체탐지(YOLOv11)·세그멘테이션(UPerNet) 모델 학습, GPU 자원 경합 방지·타일 경계 탐지 안정화",
          "인프라 - Kubernetes(kubeadm) 클러스터 구축, SaltStack 노드 스케줄링·장애 진단, Aliyun GPUShare, Nginx 구성",
        ],
      },
      {
        ...
        period: "2025.07 ~ 진행 중",
        items: [
          "아키텍처·메시징 - 모놀리식을 9개 서비스로 분리하는 MSA 마이그레이션, RabbitMQ 비동기 파이프라인, 단계별 큐 분리",
          "인증 - Envoy Gateway·Keycloak 기반 게이트웨이 레벨 인증",
          "AI 모델 - 변화탐지 AI 탐지 품질 개선(전처리·후처리)",
          "프론트엔드 - Next.js 15 FSD 기반 웹 뷰어 개발(지구/달지도 멀티 배포)",
        ],
      },
      {
        title: "사내 자동화",
