  ...
  projects: [
    ...
    },
    {
      ...
      blocks: [
        ...
        {
          label: "에어갭 데이터 정합성 확보 - 외래키·CDC·이진데이터",
          situation: "에어갭 환경에서 DB 양방향 동기화·분산 ID 발급이 필요했고, 위성 메타·추론 결과 테이블 로우 증가로 샤딩 가능성도 대비해야 했음. CDC는 slot 파손·멱등키 결함·bytea/EWKB 파싱 실패가 겹쳐 있었음",
          cause: "외래키는 샤딩 시 걸림돌이라 정합성 요구가 낮은 관측·추론 결과성 테이블은 제외하기로 함. Debezium은 slot 파손 시 전체 재동기화가 필요했고, 멱등키는 타임스탬프 포함 서명이라 재전달마다 값이 바뀌었으며, bytea·EWKB는 각각 타입 선언과 파싱 방식이 잘못돼 있었음",
          actions: [
            "정합성 요구 낮은 테이블만 외래키 제외하는 스키마로 설계",
            "폐쇄망용 Outbox+Snowflake 직접 구현, 유지 구간은 상태 판정 재정렬+자가치유로 자동 복구, 멱등키 재설계",
            "bytea는 LargeBinary+base64 디코딩, EWKB는 전용 디코더로 재정리",
          ],
          result: "샤딩 여지 확보, 이벤트 유실 0건, 23개 테이블 CDC dedup 안전 처리, 이진 데이터·지오메트리 파싱 정합성 확보",
          brief: [
            "에어갭 환경에서 DB 동기화·분산 ID 발급이 필요했는데 샤딩 대비, Debezium slot 파손, 멱등키 설계 결함, bytea·EWKB 문제가 겹쳐 있었습니다.",
            "정합성 요구 낮은 테이블만 외래키 제외하고, Outbox+Snowflake·자가치유·멱등키 재설계, LargeBinary·EWKB 디코더 전환으로 각 문제를 해소했습니다.",
          ],
        },
        {
