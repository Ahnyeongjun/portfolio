    ...
    },
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          situation: "janus(SaltStack) 기반 동기 워크플로우는 단계가 늘수록 워커 확장이 어려웠고, GPU는 파드당 1장 고정이라 대부분 유휴 상태였으며, 파드 장애 시 서비스 전체가 함께 흔들리는 구조",
          cause: "동기 방식은 각 단계가 순차 실행을 기다려야 해 병목 단계 하나가 전체를 지연시켰고, GPU 배타 점유는 여러 모델을 동시에 띄워야 하는 요건과 맞지 않았으며, 단일 파드 구조는 장애 격리가 되지 않았음",
          actions: [
            "수집→처리→카탈로깅→조율 파이프라인을 RabbitMQ 기반 비동기 구조로 전환, 단계별 큐 분리로 병목 단계만 워커 확장 가능하도록 구성",
            "Aliyun GPUShare로 GPU를 분할하며 파드 연동 생명주기까지 관리해 추론 파드 배치 자동화",
            "파드 다중화·서버 클러스터링으로 장애 격리·중앙 집중화 확보, Nginx를 게이트웨이 레벨에 두어 보안·라우팅 일원화",
          ],
          result: "단계별 큐 분리로 병목 구간만 선택적 확장, GPU 4장에서 70파드 병렬 추론, 파드 다중화·클러스터링으로 장애 격리 확보",
          brief: [
            "SaltStack 기반 동기 워크플로우는 확장이 어렵고 GPU는 파드당 1장 고정이라 대부분 유휴 상태였으며, 파드 장애 격리도 되지 않았습니다.",
            "RabbitMQ 비동기 파이프라인과 단계별 큐 분리, Aliyun GPUShare 파드 생명주기 관리, 파드 다중화·클러스터링·Nginx 게이트웨이로 안정성을 확보했습니다.",
          ],
        },
      ],
    },
