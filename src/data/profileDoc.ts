      ...
      blocks: [
        {
          ...
          situation: "국가기관 납품 시스템에서 API 성능·안정성 검증이 필요했지만, 부하테스트는 기존 sh 스크립트로만 수행되고 유닛테스트 문화도 자리잡지 않은 상태",
          cause: "sh 스크립트 기반 부하테스트는 시나리오를 세밀하게 조정하거나 에러율·처리량을 정량적으로 비교하기 어려웠고, 유닛테스트가 없어 리팩터링 시 회귀를 검증할 안전망이 없었음",
          actions: [
            "부하테스트를 k6로 전환해 VU(가상 사용자) 단위로 시나리오를 구성하고 에러율·처리량을 정량 측정",
            "API 300여 개·테이블 35개 규모 시스템에 유닛테스트를 도입해 회귀 검증 체계 확보",
          ],
          result: "k6 50VU 부하테스트로 에러율 11.22%→0%, 처리량 392→1,177 req/s 등 정량적 성능 검증 체계 확보, 유닛테스트 기반 회귀 안전망 확보",
          brief: [
            "부하테스트는 sh 스크립트로만 수행되고 유닛테스트 문화도 없어, 성능 검증과 리팩터링 안전망이 모두 부족했습니다.",
            "부하테스트를 k6로 전환하고 API 300여 개 규모 시스템에 유닛테스트를 도입해, 정량적 성능 검증과 회귀 안전망을 함께 확보했습니다.",
          ],
        },
        {
      ...
      blocks: [
        {
          ...
          situation: "팝업·iframe 등 여러 창에 컴포넌트가 흩어진 구조에서 세션·쿠키 기반 인증은 창마다 세션 상태를 공유해야 했고, 서비스를 점진적으로 분리하려 할 때도 세션에 발이 묶여 있었음",
          cause: "세션·쿠키는 서버 상태에 의존하는 방식이라 서비스를 나눌 때마다 세션 공유 방식을 다시 설계해야 했고, 인증 로직과 비즈니스 로직이 묶여 있어 장애가 생기면 관련 없는 기능까지 함께 영향을 받았음",
          actions: [
            "인증/인가 API를 세션·쿠키 기반에서 토큰 기반으로 재개발·마이그레이션",
            "한 번에 전체를 바꾸지 않고 모듈 단위로 점진적으로 분리해 전환 중 장애 전파 범위를 최소화",
          ],
          result: "서비스 분리 시 세션 공유 문제 해소, 장애 전파 최소화, 코드 재활용 증가",
          brief: [
            "세션·쿠키 기반 인증은 팝업·iframe 등 여러 창 간 세션 공유가 필요했고, 서비스를 분리하려 할 때마다 세션 상태에 발이 묶였습니다.",
            "인증/인가 API를 토큰 기반으로 재개발·마이그레이션하며 모듈 단위로 점진적으로 전환해 장애 전파 범위를 최소화하고 코드 재활용을 늘렸습니다.",
          ],
        },
        {
