      ...
      blocks: [
        {
          ...
          situation: "여러 기관에 커스텀 납품하지만 AI 모델·영상 수집·가시화·추론 등 핵심 기능과 표적·즐겨찾기·폴리곤·유저 등 지원 기능은 공통 - 자체 위성을 띄우며 기능 범위도 계속 확장 중이었음",
          cause: "로깅·인증 같은 공통 관심사까지 프로젝트마다 개별 구현되고 있어, 납품 건마다 필요한 일부만 교체·수정하려 해도 전체를 다시 손대야 했음. 모놀리식이라 기능 하나 배포에도 전체 서비스 재시작 필요",
          actions: [
            "핵심 기능과 공통 지원 기능을 도메인 단위 9개 서비스로 분리",
            "전 서비스 FastAPI 통일, Envoy Gateway로 경로별 라우팅·인증 처리",
          ],
          result: "재배포 월 10건→1건, 배포 속도 4분→30초, 신규 납품(instationgx)이 6개월 견적에서 테스트 배포까지 1개월 미만으로 단축",
          brief: [
            "여러 기관에 커스텀 납품하지만 핵심 기능은 공통이었는데, 로깅·인증까지 프로젝트마다 중복 구현돼 납품 건마다 필요한 부분만 교체하기 어려웠습니다.",
            "핵심·공통 기능을 도메인 단위 9개 서비스로 분리해 재배포 월 10건→1건, 배포 4분→30초로 줄었고, 이후 6개월 견적 프로젝트를 1개월 안에 테스트 배포까지 마쳤습니다.",
          ],
          lines: [
            "여러 기관 커스텀 납품, 핵심 기능은 공통인데 로깅·인증까지 프로젝트마다 중복 구현 - 일부만 교체하려 해도 전체를 다시 손대야 함",
            "핵심·지원 기능을 도메인 단위 9개 서비스로 분리, 전 서비스 FastAPI 통일·Envoy Gateway 라우팅",
            "재배포 월 10건→1건, 배포 4분→30초, 신규 납품(instationgx) 6개월 견적 → 1개월 미만 테스트 배포",
          ],
        },
      ],
    },
      ...
      blocks: [
        {
          ...
          situation: "부하테스트는 에러 발생 후 재발 방지용 sh 스크립트가 대부분, 유닛테스트 문화도 없었음",
          cause: "sh 스크립트는 이미 알려진 에러만 막을 뿐, 실제 유저 플로우 기반 동시 부하에서만 드러나는 문제(로그인 세션 풀림, 특정 구간 급격한 지연)는 재현할 방법이 없었음. 유닛테스트 부재로 리팩터링 회귀 검증도 불가능",
          actions: [
            "k6로 유저 플로우 시나리오를 정의하고 VU(가상 사용자) 기반으로 동시 부하 재현",
            "API 300여 개 규모 시스템에 유닛테스트 도입",
          ],
          result: "로그인 세션 풀림·구간별 지연 재현(에러율 11.22%→0%, 처리량 392→1,177 req/s), 트랜잭션 범위 문제 다수 발견·수정",
          brief: [
            "부하테스트는 sh 스크립트뿐이라 동시 부하 상황의 문제를 재현할 방법이 없었고, 유닛테스트도 없었습니다.",
            "k6로 로그인 세션 풀림·구간 지연을 잡아냈고, 유닛테스트 작성 과정에서 트랜잭션 범위 문제도 발견해 수정했습니다.",
          ],
        },
        {
          ...
          actions: [
            ...
            "유저·권한 insert를 하나의 트랜잭션으로 묶어 원자성 확보",
          ],
          result: "샤딩 여지 확보, 유저 생성 정합성 버그 근본 해결",
          brief: [
            "테이블 로우 증가로 샤딩을 염두에 둬야 했는데, 이 판단이 유저 생성에도 적용돼 유저·권한 insert가 별도 트랜잭션이라 정합성이 깨지는 버그가 있었습니다.",
            "정합성 요구 낮은 테이블만 외래키 제외, 유저 생성은 단일 트랜잭션으로 묶어 원자성을 확보했습니다.",
          ],
        },
        {
          ...
          cause: "Debezium CDC는 slot 파손 시 전체 재동기화가 필요해 안정성이 낮았고, 상태 판정 우선순위 오류로 WAL 완전 유실 상황도 '비활성'으로 오판해 필요한 slot 재생성을 건너뛰는 버그도 있었음. 멱등키는 payload 서명에 타임스탬프 컬럼까지 포함돼 동일 이벤트도 재전달마다 다른 서명이 생성됐고, 원본 payload를 얕은 참조로 들고 있어 처리 중 오염되는 문제도 있었음",
          actions: [
            "폐쇄망 동기화 용도로는 Debezium CDC를 제거하고 Outbox 라이브러리 직접 구현, Snowflake 알고리즘으로 worker ID에 망 정보 인코딩",
            "유지해야 하는 CDC 파이프라인은 상태 판정 순서를 재정렬하고, 강제종료→slot 삭제→재생성→검증 자가치유 함수로 자동화(연속 3회 실패 시에만 트리거)",
            "휘발성 컬럼을 제외하고 정렬해 정규화하는 서명 로직으로 멱등키를 재설계, 원본 payload는 deepcopy로 참조 분리",
          ],
          result: "이벤트 유실 0건, 전역 유일 ID 발급 확보, 23개 테이블 CDC dedup 안전 처리",
          brief: [
            "Debezium replication slot이 반복 파손됐고, 재전달된 CDC 이벤트가 멱등키 설계 결함(타임스탬프 포함 서명)으로 중복 반영됐습니다.",
            "폐쇄망용은 Outbox+Snowflake로 Debezium 의존을 제거하고, 유지 구간은 자가치유 스크립트로, 이벤트 중복은 멱등키 재설계로 각각 안정화했습니다.",
          ],
        },
        {
          label: "CDC 이진 데이터·지오메트리 정합성 확보",
          situation: "공지 첨부파일·팝업 이미지 등 bytea 컬럼이 CDC 경로를 거치며 이미지가 깨졌고, PostGIS EWKB도 지오메트리 타입·엔디안 조합에 따라 파싱이 간헐적으로 실패",
          cause: "bytea 컬럼이 SQLAlchemy 모델에 String으로 잘못 선언돼 Postgres 원본과 타입이 불일치했고, EWKB는 SRID 유무를 바이트 오프셋·비트마스크로 직접 잘라내는 임시방편을 쓰고 있었음. 좌표 소수점 표현도 Go·Python 양쪽에서 통일돼 있지 않아 CDC 멱등키 매칭까지 깨질 수 있는 구조였음",
          actions: [
            "bytea 컬럼을 LargeBinary로 재정의하고 base64 디코딩으로 정정, Debezium 소스 설정에 이진 처리 모드를 명시하고 실패 시 hex 폴백 추가",
            "EWKB는 전용 디코더(ewkb)로 교체해 SRID 파싱을 라이브러리에 위임, 좌표 소수점 자릿수를 16자리로 Go·Python 양쪽 통일",
          ],
          result: "이미지·첨부파일 CDC 동기화 정상화, 수동 바이너리 파싱 코드 제거, CDC 멱등키 매칭 안정성 확보",
          brief: [
            "bytea 컬럼 타입 불일치와 잘못된 디코딩으로 이미지가 깨졌고, EWKB도 수동 비트마스킹으로 파싱해 간헐적으로 실패했습니다.",
            "bytea는 LargeBinary+base64 디코딩으로, EWKB는 전용 디코더로 각각 재정리해 CDC 데이터 정합성을 확보했습니다.",
          ],
        },
        {
          ...
          brief: [
            "SaltStack job 폴링은 프로세스가 죽으면 작업이 진행 상태에 무한히 머물렀고, 고정 주기 폴링으로 대기 낭비도 있었습니다.",
            "폴링 구조 자체를 없애고 RabbitMQ 이벤트 발행-구독으로 전환했습니다. 이후 워크플로우 대부분이 단일 흐름이라는 걸 확인해 Airflow류 오케스트레이션과 비교 검토 중입니다.",
          ],
        },
      ],
    },
