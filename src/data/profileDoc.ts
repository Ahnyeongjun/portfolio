      ...
      blocks: [
        {
          ...
          situation: "국가기관 납품 시스템에서 API 성능·안정성 검증이 필요했지만, 부하테스트는 에러가 발생한 뒤 동일 에러 재발을 막기 위해 사후적으로 추가되는 sh 스크립트가 대부분이었고, 유닛테스트 문화도 자리잡지 않은 상태",
          cause: "sh 스크립트는 이미 알려진 에러를 막는 방어 코드일 뿐이라, 실제 유저 플로우 기반의 동시 부하 상황에서만 드러나는 문제(로그인 세션이 풀리는 현상, 특정 구간만 유난히 느려지는 현상)는 애초에 재현할 방법이 없었음. 유닛테스트가 없어 리팩터링 시 회귀를 검증할 안전망도 없었고, 기존 코드는 비즈니스 로직 비중이 낮고 보일러플레이트가 많아 테스트를 짜려면 그 구조부터 분석해야 했음",
          actions: [
            "부하테스트를 k6로 전환해 실제 유저 플로우를 시나리오로 정의하고 VU(가상 사용자) 수를 늘려가며 동시 부하를 재현",
            "API 300여 개·테이블 35개 규모 시스템에 유닛테스트를 도입, 보일러플레이트가 많은 기존 코드를 분석하며 회귀 검증 체계 확보",
          ],
          result: "k6로 로그인 세션이 풀리는 문제, 데이터·요청량이 늘수록 급격히 느려지는 구간을 정량적으로 재현·특정(에러율 11.22%→0%, 처리량 392→1,177 req/s), 유닛테스트 작성 과정에서 트랜잭션 범위 문제도 여러 건 추가로 발견해 수정",
          brief: [
            "부하테스트는 에러가 난 뒤 동일 에러만 막는 sh 스크립트가 대부분이었고, 유닛테스트도 없어 동시 부하나 리팩터링 시 발생하는 문제를 사전에 잡을 방법이 없었습니다.",
            "k6로 유저 플로우 기반 부하테스트를 도입해 로그인 세션 풀림·특정 구간 지연을 잡아냈고, 유닛테스트 작성 과정에서 트랜잭션 범위 문제도 여러 건 추가로 발견해 수정했습니다.",
          ],
        },
        {
          label: "에어갭 환경 개선 및 망연계",
          situation: "물리 망분리(에어갭) 환경에서 외부망↔폐쇄망 DB 양방향 동기화와 분산 ID 발급이 필요했고, 위성 메타·추론 결과 테이블 로우 증가로 향후 샤딩 가능성도 염두에 둬야 했음. 유지되는 CDC 파이프라인에서는 Debezium replication slot이 반복 파손되고, 재전달 이벤트가 멱등키 설계 결함으로 중복 반영됐으며, bytea 컬럼과 지오메트리 파싱에서도 각각 타입 불일치·간헐적 파싱 실패가 있었음",
          cause: "외래키는 참조 대상이 다른 샤드로 옮겨가는 순간 걸림돌이 되는데 관측·추론 결과성 데이터는 정합성이 깨져도 치명적이지 않았음(다만 이 판단이 유저 생성처럼 정합성이 중요한 경로에도 그대로 적용돼 별도 트랜잭션 정합성 버그가 있었음). Debezium은 slot 파손 시 전체 재동기화가 필요해 안정성이 낮았고, 멱등키는 타임스탬프까지 서명에 포함돼 재전달마다 값이 달라졌으며, bytea 컬럼은 ORM에 String으로 잘못 선언되고 EWKB는 수동 비트마스킹으로 파싱해 둘 다 CDC 경로의 정합성을 흔들었음",
          actions: [
            "정합성 요구가 낮은 테이블에 한해 외래키 없는 스키마로 설계하고, 유저 생성처럼 정합성이 중요한 경로는 트랜잭션 경계로 원자성을 재확보",
            "폐쇄망 동기화용 Outbox 라이브러리 직접 구현(Snowflake 알고리즘으로 망 정보 인코딩), 유지되는 CDC 구간은 상태 판정 순서 재정렬 + 자가치유 함수로 자동 복구",
            "휘발성 컬럼을 제외한 정규화 서명으로 멱등키 재설계, bytea 컬럼은 LargeBinary+base64 디코딩으로, EWKB는 전용 디코더로 각각 재정리",
          ],
          result: "샤딩 여지 확보, 이벤트 유실 0건, 23개 테이블 CDC dedup 안전 처리, 이진 데이터·지오메트리 파싱 정합성 확보",
          brief: [
            "에어갭 환경에서 DB 동기화·분산 ID 발급이 필요했는데 Debezium slot 파손, 멱등키 설계 결함, bytea 타입 불일치, EWKB 파싱 실패가 겹쳐 있었습니다.",
            "외래키 없는 스키마+트랜잭션 경계, Outbox+Snowflake, 멱등키 재설계, LargeBinary·EWKB 디코더 전환으로 각 문제를 계층별로 해소했습니다.",
          ],
        },
        {
          label: "서비스 안정화를 위한 인프라 업데이트",
          situation: "janus(SaltStack) 워크플로우는 DB job 테이블(00 대기·01 진행·02 완료·03 실패)을 폴링하며 상태를 갱신했는데, 처리 중이던 서비스가 완전히 죽으면 작업이 02/03으로 전이되지 못하고 01(진행)에 무한히 머무는 문제가 있었음. 고정 주기 폴링(1분)도 낭비가 있어, 작업이 폴링 직후 등록되면 다음 폴링까지 최대 1분을 그대로 대기했음",
          cause: "job 상태 전이를 처리 프로세스 본인이 책임지는 구조라 그 프로세스가 죽으면 상태값만으로는 복구할 방법이 없었음 - 타임아웃·재시도 로직을 얹어 땜질할 수도 있었지만, 폴링 기반 상태 관리 자체가 근본적으로 이 문제를 안고 있다고 판단해 구조를 바꾸기로 결정. 여러 작업을 병렬로 처리하는 데도 job 테이블에 여러 건을 등록하는 방식보다, 이벤트 하나를 발행하고 여러 워커가 구독해 각자 처리하는 편이 구조적으로 더 효율적이었음",
          actions: [
            "타임아웃·재시도로 폴링 구조를 땜질하는 대신, 문제의 근본 원인(폴링 기반 상태 관리)을 없애기로 하고 RabbitMQ 기반 비동기 구조로 전환",
            "job 테이블에 여러 건을 등록하는 방식 대신 이벤트 발행-구독 구조로 재설계해 여러 워커가 하나의 이벤트를 각자 병렬로 처리하도록 구성, 단계별 큐 분리로 병목 단계만 워커 확장 가능하도록 함",
            "Aliyun GPUShare로 GPU를 분할하며 파드 연동 생명주기까지 관리해 추론 파드 배치 자동화, 파드 다중화·서버 클러스터링으로 장애 격리 확보",
          ],
          result: "죽은 프로세스로 인해 작업이 진행 상태에 무한히 머무는 문제 근본 해결, 폴링 주기로 인한 대기 낭비 제거, 단계별 큐 분리로 병목 구간만 선택적 확장, GPU 4장에서 70파드 병렬 추론. 다만 실제 워크플로우 대부분이 다방향 분기보다 단일 흐름에 가까웠고 '발행 후 끝'보다 각 단계가 확실히 완료·검수된 뒤 다음 단계로 넘어가는 편이 낫다는 의견이 나와, 현재 RabbitMQ 이벤트 방식과 Airflow류 워크플로우 오케스트레이션을 비교 검토 중",
          brief: [
            "SaltStack 기반 job 폴링은 서비스가 죽으면 작업이 진행 상태에 무한히 머물렀고, 고정 주기 폴링 때문에 작업 등록 직후에도 대기 낭비가 있었습니다.",
            "타임아웃 땜질 대신 폴링 구조 자체를 없애기로 하고 RabbitMQ 기반 이벤트 발행-구독으로 전환했으며, 이후 워크플로우가 대부분 단일 흐름이라는 걸 확인하고 Airflow류 오케스트레이션과 비교 검토 중입니다.",
          ],
        },
      ],
    },
      ...
      blocks: [
        {
          ...
          situation: "인증/인가 기능을 별도 서비스로 분리하는 과정에서, 세션·쿠키 기반 인증은 서버(도메인)마다 세션이 분리돼 있어 사용자가 다른 서버로 넘어가면 재로그인이 필요했음",
          ...
          actions: [
            "인증/인가 API를 세션·쿠키 기반에서 토큰 기반으로 재개발·마이그레이션해 서버 간 재로그인 문제 해소",
          ...
          result: "서버 간 재로그인 문제 해소, 세션 가시성 확보, 장애 전파 최소화",
          brief: [
            "인증/인가를 분리하며 세션·쿠키 기반은 서버마다 세션이 갈려 재로그인이 필요했고, 토큰만 쓰면 반대로 활성 세션을 추적할 수 없었습니다.",
            "토큰 기반으로 마이그레이션해 재로그인 문제를 해소하고, Redis 역색인으로 세션 가시성까지 함께 확보하며 모듈 단위로 점진적으로 전환했습니다.",
          ],
        },
        {
