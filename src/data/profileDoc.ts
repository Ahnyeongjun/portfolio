...
export const PROFILE = {
  ...
  projects: [
    ...
    },
    {
      ...
      blocks: [
        {
          label: "인증·인가 체계 설계 (Spring Security + AOP + Redis 세션)",
          situation: "엔드포인트가 늘수록 컨트롤러마다 권한 체크 코드가 흩어져 누락·중복 위험이 커졌고, 동시 로그인 통제·로그인 시도 공격 방어도 필요했으나 토큰 단방향 저장만으로는 '이 사용자가 몇 세션 접속 중인가'를 답할 수 없었음",
          cause: "인증과 인가 로직이 분리되지 않고 각 컨트롤러에 개별 구현되어 있어 신규 API마다 권한 체크를 매번 새로 작성해야 했고, 토큰 저장 구조도 검증만 가능할 뿐 사용자 기준 역조회가 불가능했음",
          actions: [
          ...
          result: "인가 로직 중앙화로 권한 적용 누락 구조적 방지, 세션 상태를 앱 메모리가 아닌 Redis로 외부화해 서버 증설과 무관하게 동작하는 구조, 해제 배치 등 운영 코드 0",
          brief: [
            "엔드포인트가 늘수록 컨트롤러마다 권한 체크 코드가 흩어져 있었고, 토큰 단방향 저장만으로는 사용자별 활성 세션 수를 답할 수 없어 동시 로그인 통제·로그인 시도 방어도 불가능했습니다.",
            "인증은 Spring Security 필터 체인에서, 인가는 커스텀 어노테이션+AOP로 선언적으로 분리하고, user→token 역색인을 Redis에 저장해 세션을 즉시 조회·통제하며 로그인 실패는 IP·계정 단위로 카운트해 TTL 자동 만료로 차단하도록 만들었습니다.",
          ],
        },
        {
          label: "다운로드 엔드포인트 통합",
